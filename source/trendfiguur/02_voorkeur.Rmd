...
grid.arrange(
  ...
  create_figure(
    ...
    geom_blank(data = data.frame(x = max(ds$x), mu = 1.45)),
  create_figure(
    dataset = ds_potential, reference = "lines + text", ci = "gradiënt"
  ) +
    ggtitle("gradiënt") +
    geom_blank(data = data.frame(x = max(ds$x), mu = 1.45)),
  ncol = 1
...
inla.posterior.sample(n = 1e3, m_type_trend) |>
  ...
  mutate(
    onzekerheid = factor(
      .data$onzekerheid, levels = c("band", "gradiënt", "geen"),
...
Om het onderscheid te kunnen maken tussen "stabiel" en "onbekend" moeten we definiëren wat een een belangrijke toename en een belangrijke afname zijn.

...
We hebben daarom de correcte interpretatie aangeduidt in de figuren in §\@ref(voor-na).
...
ds_eigen |>
  distinct(
    .data$id, .data$index, .data$gradient, .data$onzekerheid, lijnen = .data$lijn,
    `geannoteerde lijnen` = .data$lijn_tekst, .data$symbool,
    `gekleurd symbool` = .data$kleur
  ) |>
  mutate(
...
Tijdens de tweede reeks steegt dit tot drie kwart van de gevallen.

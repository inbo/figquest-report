# Interpretatie van trendfiguren

Voordat de deelnemers hun voorkeur konden aangeven, schotelden we hun eerste drie eenvoudige trendfiguren voor.
Hierbij gebruikten we de stijl van figuur \@ref(fig:y-as) met de Y-as uitgedrukt als index (linkerfiguur).
...
1)  "matige toename": significante toename met betrouwbaarheidsinterval volledig tussen de referentie en de belangrijke toename.
2)  "stabiel": niet significante wijziging die enkel de referentie omvat en volledig tussen een belangrijke afname en een belangrijke toename ligt.
3)  "mogelijke toename": niet significante toename met betrouwbaarheidsinterval dat zowel de referentie als een belangrijke toename omvat.

Aangezien dit eenvoudige figuren waren, zonder aanduiding van betrouwbaarheidsintervallen of referentielijnen, konden deelnemers onmogelijk met zekerheid weten wat de correcte indeling was.
Nadat de deelnemers hun voorkeur konden opgeven, vroegen we hen om opnieuw drie figuren te interpreteren.
Hierbij krijgt elke deelnemer dezelfde drie tijdreeksen voorgeschoteld, ditmaal in de voorkeursstijl van de deelnemer zelf.
We vertelden de deelnemers niet dat ze exact dezelfde vragen kregen als bij de start van de bevraging.
De deelnemers hebben bijgevolg zonder aankondiging dezelfde drie vragen twee keer beantwoord, met enkel het verschil in stijl van de figuur.
...
Een derde van de deelnemers wijzigt zijn mening van "hoger" naar het correcte "onduidelijk" tijdens de tweede reeks (fig. \@ref(fig:interpretatie-mogelijk2)).
...
interpretation |>
  ...
  complete(.data$id, .data$reeks, fill = list(score = "geen antwoord")) |>
...
(ref:interpretatie-matig-sankey) Sankey diagram van de antwoorden van de deelnemers voor beide reeksen voor de index met "matige toename" als correcte interpretatie.

```{r interpretatie-matig-sankey, fig.cap = "(ref:interpretatie-matig-sankey)", warning = FALSE}
interpretation |>
  filter(
    !.data$achteraf | .data$eigen, .data$waarheid == "matige toename"
  ) |>
  ...
  pivot_wider(
    names_from = "reeks", values_from = "score", values_fill = "geen antwoord"
  ) |>
  inner_join(background, by = "id") |>
  count(.data$eerste, .data$tweede, .data$type) |>
  ggplot(aes(axis1 = eerste, axis2 = tweede, y = n)) +
  ...
  scale_x_continuous(
    breaks = 1:3, labels = c("eerste reeks", "tweede reeks", "type deelnemer")
  )
```
...
(ref:interpretatie-stabiel-sankey) Sankey diagram van de antwoorden van de deelnemers voor beide reeksen voor de index met "stabiel" als correcte interpretatie.

```{r interpretatie-stabiel-sankey, fig.cap = "(ref:interpretatie-stabiel-sankey)", warning = FALSE}
interpretation |>
  filter(
    !.data$achteraf | .data$eigen, .data$waarheid == "stabiel"
  ) |>
...
(ref:interpretatie-mogelijk-sankey) Sankey diagram van de antwoorden van de deelnemers voor beide reeksen voor de index met "mogelijke toename" als correcte interpretatie.

```{r interpretatie-mogelijk-sankey, fig.cap = "(ref:interpretatie-mogelijk-sankey)", warning = FALSE}
interpretation |>
  filter(
    !.data$achteraf | .data$eigen, .data$waarheid == "mogelijke toename"
  ) |>
...
De overgrote meerderheid van de deelnemers verkoos om betrouwbaarheidsintervallen en referentielijnen.
...
Strikt genomen onderzochten we met welke stijl van figuren de deelnemers het best instaat zijn om tot een gelijkwaardige interpretatie als de makers van de figuur te komen.
Het correcte antwoord selecteren we bijgevolg louter op basis van de volledige set van gegevens, inclusief de kennis over onzekerheid en referentielijnen.
Indien we de stijl van de figuur in rekening brengen, zou de correcte interpretatie van de eerste reeks figuren steeds "geen idee" zijn.

```{r verklaring-beste, fig.cap = "Gemiddelde kans op een correcte interpretatie in functie van het gebruik van symbolen en de schaal van y-as. De tweede reeks valt grotendeels samen met het toevoegen van referentielijnen en weergave van onzekerheid."}
voorspelling |>
  mutate(
    reeks = factor(.data$tweede_reeks, 0:1, c("eerste reeks", "tweede reeks")),
    stijl = interaction(.data$index, .data$symbool) |>
      factor(
        levels = c("1.0", "1.1", "0.0", "0.1"),
        labels = c(
          "index\nzonder symbool", "index\nmet symbool",
          "procentuele\nwijziging\nzonder symbool",
          "procentuele\nwijziging\nmet symbool"
        )
      ),
  ...
  ggplot(aes(x = reeks, y = median, fill = stijl, shape = `y-as`)) +
  geom_hline(yintercept = 0.5, linetype = 2) +
  geom_hline(yintercept = c(0.1, 0.9), linetype = 4) +
  geom_hline(yintercept = c(0.25, 0.75), linetype = 3) +
  stat_fan(
    aes(link_sd = sd), geom = "rect", position = position_dodge(width = 1),
    link = "logit"
  ) +
  scale_y_continuous(
    "kans op een correct antwoord", breaks = c(0, 0.1, 0.25, 0.5, 0.75, 0.9, 1),
...
```{r verklaring-parameter, fig.cap = "Gewogen effect van de modelparameters.", eval = FALSE}
model_fit |>
  transmute(
    .data$model_id,
    .data$weight,
